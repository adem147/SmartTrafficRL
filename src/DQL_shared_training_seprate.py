...
SAVE_EVERY = 100 
MODEL_FILE = os.path.join(RESULTS_DIR, "dqn_shared_seprate_intersections.pt")
...
def get_local_state(tls_id):
    ...
    return np.array([ns/10, ew/10], dtype=np.float32)
...
def run_episode(policy, target, optimizer, memory):
    ...
    while step < MAX_STEPS:
        ...
        if step % T_DECISION == 0:
            ...
            # choose action per agent
            for tls in TLS_IDS:
                a = choose_action(states[tls], policy)
                actions[tls] = a
                traci.trafficlight.setPhase(tls, ACTIONS[a])
...
if __name__ == "__main__":
    ...
    with open(os.path.join(RESULTS_DIR, f"DQN_shared_seprate_results_{EPISODES}.pkl"), "wb") as f:
        pickle.dump(data, f)
